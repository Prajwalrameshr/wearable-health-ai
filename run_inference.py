...
from src.gmm import load_gmm_model, run_gmm_pipeline
from src.hmm_model import load_hmm_model, run_hmm_pipeline, run_soft_probability_hmm
from src.kmeans import load_kmeans_model, run_kmeans_pipeline
from src.preprocessing import CLUSTER_FEATURE_COLUMNS, load_scaler, preprocess_for_modeling, transform_features
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run wearable health analytics inference on a single-user or longitudinal CSV.")
...
def _attach_kmeans_probabilities(labeled_df: pd.DataFrame, kmeans_out: dict[str, Any]) -> pd.DataFrame:
    ...
    for cluster_id, raw_state_label in label_map.items():
        ...
        enriched[probability_column] = np.maximum(
            enriched[probability_column].to_numpy(dtype=float), cluster_probabilities[:, int(cluster_id)]
        )
    probability_columns = [
        f"prob_{state.lower().replace(' ', '_')}" for state in STANDARD_STATES if f"prob_{state.lower().replace(' ', '_')}" in enriched.columns
    ]
    enriched["state_confidence"] = enriched[probability_columns].max(axis=1)
    enriched["dominant_probability_state"] = (
        enriched[probability_columns].idxmax(axis=1).str.replace("prob_", "", regex=False).str.replace("_", " ").str.title()
    )
    return enriched
...
def load_model_outputs(feature_df: pd.DataFrame, model_type: str) -> tuple[pd.DataFrame, dict[str, Any], dict[str, Any]]:
    ...
    if scaler is not None:
        scaled_cluster_df[CLUSTER_FEATURE_COLUMNS] = transform_features(feature_df, scaler, CLUSTER_FEATURE_COLUMNS)
    else:
        outputs = preprocess_for_modeling(feature_df)
        scaler = outputs["scaler"]
        scaled_cluster_df = outputs["scaled_feature_matrix"]

    if model_type == "gmm":
        fitted_gmm = load_gmm_model()
        cluster_out = run_gmm_pipeline(
            scaled_cluster_df, feature_columns=CLUSTER_FEATURE_COLUMNS, fitted_model=fitted_gmm, save_trained_model=(fitted_gmm is None)
        )
        labeled_df = cluster_out["labeled_df"].copy()
    elif model_type == "kmeans":
        fitted_kmeans = load_kmeans_model()
        cluster_out = run_kmeans_pipeline(
            scaled_cluster_df, feature_columns=CLUSTER_FEATURE_COLUMNS, fitted_model=fitted_kmeans, save_trained_model=(fitted_kmeans is None)
        )
        labeled_df = _attach_kmeans_probabilities(cluster_out["labeled_df"], cluster_out)
    else:
        raise ValueError(f"Unsupported model_type: {model_type}")

    fitted_hmm = load_hmm_model()
    hmm_out = run_hmm_pipeline(
        labeled_df, observation_column=_get_observation_column(model_type), n_components=3, fitted_model=fitted_hmm, save_model=(fitted_hmm is None)
    )
...
def _build_state_probabilities(latest_row: pd.Series) -> dict[str, float]:
    ...
    for state in STANDARD_STATES:
        val = latest_row.get(f"prob_{state.lower().replace(' ', '_')}", 0.0)
        if isinstance(val, pd.Series):
            val = val.iloc[0]
        result[state] = round(float(val) * 100.0, 1)
    return result
...
def _build_model_feature_influence(model_type: str, cluster_out: dict[str, Any], latest_row: pd.Series) -> dict[str, float]:
    ...
    if model_type == "gmm" and hasattr(cluster_out.get("model"), "means_"):
        ...
        return {}
    return {
        feature: round(abs(float(value)), 4)
        for feature, value in sorted(zip(feature_columns, influence), key=lambda item: abs(item[1]), reverse=True)
    }
...
def compute_metrics(model_type: str, cluster_out: dict[str, Any], hmm_out: dict[str, Any]) -> dict[str, Any]:
    ...
    if model_type == "gmm":
        ...
        selected_row = (
            selection_df.loc[selection_df["k"] == selected_k].iloc[0]
            if selection_df is not None and not selection_df.empty and selected_k in selection_df["k"].tolist()
            else None
        )
        return {
            ...
            "bic": round(float(selected_row["bic"]), 4) if selected_row is not None and "bic" in selected_row else None,
            "aic": round(float(selected_row["aic"]), 4) if selected_row is not None and "aic" in selected_row else None,
...
def compute_state(model_type: str, merged_df: pd.DataFrame, hmm_out: dict[str, Any], cluster_out: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    def apply_risk(row: pd.Series) -> pd.Series:
        risk = calculate_risk_score(
            row,
            state=str(row.get(state_column, "Unknown")),
            state_confidence=float(row.get("state_confidence", 0.0)),
            state_duration=int(row.get(duration_column, 1)),
        )
        stress_level = (
            "Low" if str(row.get(state_column, "Baseline")) == "Recovery"
            else "Moderate" if str(row.get(state_column, "Baseline")) == "Baseline"
            else "High"
        )
        return pd.Series({
    ...
    risk = calculate_risk_score(
        ...
        state_confidence=float(latest_row.get("state_confidence", 0.0)),
        state_duration=int(latest_row.get(duration_column, 1)),
    )
    ...
    previous_state = (
        str(ordered.iloc[-2].get(state_column, latest_row.get(state_column, "Unknown")))
        if len(ordered) > 1
        else str(latest_row.get(state_column, "Unknown"))
    )
    temporal_trend = (
        "Stable" if previous_state == str(latest_row.get(state_column, "Unknown"))
        else "Improving" if previous_state == "Strain" and str(latest_row.get(state_column, "Unknown")) in {"Baseline", "Recovery"}
        else "Deteriorating" if previous_state in {"Recovery", "Baseline"} and str(latest_row.get(state_column, "Unknown")) == "Strain"
        else risk["early_warning"].get("trend", "Stable")
    )
    analysis_result = {
...
def run_pipeline(csv_path: str, output_path: str = str(DEFAULT_OUTPUT_PATH), model_type: str = "gmm", **kwargs) -> dict[str, Any]:
    ...
    merge_columns = [
        "user_id", "date", hmm_bundle["state_column"], f"{hmm_bundle['state_column']}_label", hmm_bundle["duration_column"]
    ]
    ...
    final_results_df, analysis_result = compute_state(model_type, merged_df, hmm_bundle, cluster_out)
    ...
    return {
        ...
        "hmm_out": hmm_bundle,
        "state_distribution": hmm_bundle["state_distribution"],
        "transition_matrix": hmm_bundle["transition_matrix"],
        "cluster_summary": cluster_out["cluster_summary"],
    }


def main() -> None:
    args = parse_args()
    results = run_pipeline(csv_path=args.csv_path, output_path=args.output, model_type=args.model)
    print(f"Inference complete! Saved output to {results['output_path']}")
